...
export const TagDescriptions: Record<OrgTags, string> = {
  ...
  [OrgTags.SOCIAL]:
    "Social rides for when you're just looking for a cruise with friends and maybe a beverage or some snacks at the end.",
  [OrgTags.MOUNTAIN]:
    'Groups that offer mountain biking, often on singletrack at a local park like Theodore Wirth.',
  [OrgTags.MEMBERS_ONLY]:
    'Groups tagged "members-only" have at lease some rides restricted to dues-paying members. They may also have open rides, check with the group for details.',
  [OrgTags.FAMILY]: 'Groups that specifically advertise family-friendly rides.',
  [OrgTags.FTW]:
    'Groups tagged "FTW" (Femme, Trans, Women) have at lease one ride or activity advertised as such. They might have other open rides, check with the group for details.',
};
...
export function TagNavigator(params: { activeTag?: OrgTags; className?: string }) {
  ...
  return (
    ...
    >
      {allTags.map((tag) => (
        <Tag key={tag} tag={tag} active={tag === activeTag} className="mr-2 mb-2" />
      ))}
    </nav>
  );
}
